...
it("voronoi.renderCell(i, context) is a noop for coincident points", () => {
  ...
  assert.equal(voronoi.renderCell(3, {}), undefined);
});
...
it("voronoi.renderCell(i, context) handles midpoint coincident with circumcenter", () => {
  ...
  assert.equal((voronoi.renderCell(0, context), context.toString()), `M-1,-1L0.5,-1L0.5,0.5L-1,0.5Z`);
  assert.equal((voronoi.renderCell(1, context), context.toString()), `M2,-1L2,2L0.5,0.5L0.5,-1Z`);
  assert.equal((voronoi.renderCell(2, context), context.toString()), `M-1,2L-1,0.5L0.5,0.5L2,2Z`);
});

it("voronoi.contains(i, x, y) is false for coincident points", () => {
  let voronoi = Delaunay.from([[0, 0], [1, 0], [0, 1], [1, 0]]).voronoi([-1, -1, 2, 2]);
  assert.equal(voronoi.contains(3, 1, 0), false);
  assert.equal(voronoi.contains(1, 1, 0), true);
});
...
it("voronoi.update() updates the voronoi", () => {
  ...
  assert.deepEqual(p, [[-500, 500], [-500, -140], [-240, -140], [-140, 60], [-140, 500], [-500, 500]]);
});
...
it("voronoi.update() updates a degenerate voronoi", () => {
  ...
  let voronoi = delaunay.voronoi([-500, -500, 500, 500]);
  assert.deepEqual(voronoi.cellPolygon(0), [ [ 500, -500 ], [ 500, 500 ], [ -500, 500 ], [ -500, -500 ], [ 500, -500 ] ]);
  assert.equal(voronoi.cellPolygon(1), null);
  for (let i = 0; i < delaunay.points.length; i++) {
...
it("zero-length edges are removed", () => {
   ...
   assert.equal(voronoi1.cellPolygon(0).length, 4);
   const voronoi2 = Delaunay.from([[10, 10], [20, 10]]).voronoi([0, 0, 30, 20]);
   assert.deepEqual(voronoi2.cellPolygon(0), [[0, 20], [0, 0], [15, 0], [15, 20], [0, 20]]);
});
...
it("voronoi neighbors are clipped", () => {
   ...
   assert.deepEqual([...voronoi.neighbors(0)].sort(), [1, 2]);
   assert.deepEqual([...voronoi.neighbors(1)].sort(), [0, 2]);
   assert.deepEqual([...voronoi.neighbors(2)].sort(), [0, 1, 4]);
   assert.deepEqual([...voronoi.neighbors(3)].sort(), []);
   assert.deepEqual([...voronoi.neighbors(4)].sort(), [2]);
   assert.deepEqual([...voronoi.neighbors(5)].sort(), []);
});
...
it("unnecessary points on the corners are avoided (#88)", () => {
  for (const [points, lengths] of [
    ...
    assert.deepEqual([...voronoi.cellPolygons()].map(d => d.length), lengths);
  }
});
...
it("a degenerate triangle is avoided", () => {
  ...
  assert.equal(voronoi.cellPolygon(0).length, 4);
});
...
it("cellPolygons filter out empty cells and have the cell index as a property", () => {
  ...
  assert.deepEqual([...voronoi.cellPolygons()], [
