...
tape("voronoi.findIndex(x, y) returns the index of the cell that contains the specified point", test => {
  let voronoi = Delaunay.from([[0, 0], [1, 0], [0, 1], [1, 1]]).voronoi([-1, -1, 2, 2]);
  test.deepEqual(voronoi.findIndex(0.25, 0.25), 0);
});

tape("voronoi.find(x, y) returns the cell that contains the specified point", test => {
  let voronoi = Delaunay.from([[0, 0], [1, 0], [0, 1], [1, 1]]).voronoi([-1, -1, 2, 2]);
  test.equal(voronoi.find(0.25, 0.25), voronoi.cells[0]);
});
